<template>
  <div class="cart-box">
    ...
    <div class="empty" v-if="!state.list.length">
      ...
      <van-button round color="#518a4e" type="primary" @click="goTo" block
        >前往选购</van-button
      >
    </div>
  </div>
...
const onChange = async (value, detail) => {
  if (value > 5) {
    showFailToast("超出单个商品最大购买数量");
    return;
  }
  if (value < 1) {
    showFailToast("商品个数不得小于0");
    return;
  }
  /**
   ...
   */
  if (
    state.list.find((item) => item.cartItemId == detail.name)?.goodsCount ==
    value
  )
    return;
